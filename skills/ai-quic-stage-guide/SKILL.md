...
21. 仓库根目录下的 `archived/` 目录视为用户专属归档区。任何情况下都不得修改、覆盖、删除、移动、重命名、复制或新增其中的文件和内容；只允许读取。
...
8. 新阶段或新验证方案落地时，必须保留现有单元测试、本机回环和 `topo.py`/`Makefile` 入口；不要为了接新测试而替换掉旧链路。
9. 如果新的网络验证会明显扰动当前稳定的 `topo.py` profile，优先新增独立的 `topo_*.py` 或阶段专用拓扑脚本，而不是覆写已稳定入口。
10. 推进任务时要及时通读并更新 `README.md` 全文，把总览、能力矩阵、测试入口、阶段状态和完成度同步到当前真实进度，而不是只改局部章节。
11. 新增或修改公共函数时，优先在 `include/` 的声明处补“功能 + 返回值”注释；不要只在实现内部留零散说明。
12. 对外 API 除了头文件注释外，还要同步更新根目录 `API.md`，逐函数记录用途、参数语义和返回值含义。
